...
- **Queued unused IAM-role finding pipeline** - `AriaAccessAnalyzerStateMachine` starts an unused-role dispatcher in parallel with the retained internal/external poll loops. The dispatcher uses `ListFindingsV2` service-side filters for active `UnusedIAMRole` findings with an `AWS::IAM::Role` resource, preserves the existing Identity Center/AAM tracked-role restriction, and queues only new or changed summaries. A single-concurrency SQS worker retrieves full details at the configured strict global RPS, so a large backfill survives Lambda timeouts and scheduler overlap without multiplying `GetFindingV2` calls. The worker uses a dedicated work DLQ, while a DynamoDB lease and enqueue checkpoint prevent overlapping schedules from duplicating the backlog.
- **Separate least-privilege Access Analyzer roles** - the retained internal/external poller, unused-role dispatcher, and unused-role detail worker have distinct local execution roles. In cross-account deployments, `aria-access-analyzer-poller-role.yaml` creates matching delegated-admin roles: the dispatcher can only list the configured unused analyzer, and the worker can only retrieve its finding details. See [delegated-admin-role-setup.md](source/accessanalyzerpoller/delegated-admin-role-setup.md).
- **Fixed-rate controls for each path** - `requestsPerSecond` continues to pace the internal/external poller. `unusedRoleWorkerRequestsPerSecond` sets the unused-role worker rate. Because that worker has Lambda reserved concurrency fixed at one, its rate is a true global cap rather than a per-invocation estimate. The SQS event source has no separate concurrency cap because that conflicts with the Lambda reservation. Tune it only after monitoring queue age and throttling. The existing `maxPollAttempts` setting applies to the internal/external Step Functions loops.
